...
The goal is to make the current state of the product visible so that ISMS practitioners, SaaS builders, potential collaborators, and people interested in AI-driven development can review the direction, run the app locally, try the demo, and give feedback.
...
The current implementation no longer assumes Supabase/Postgres, Supabase CLI, Supabase migrations, or Service Role Key usage.
...
This public package is limited to files that help readers inspect, run, and evaluate the product. It is not intended to include unrelated work logs, local machine state, generated QA output, local databases, secrets, or tokens.
