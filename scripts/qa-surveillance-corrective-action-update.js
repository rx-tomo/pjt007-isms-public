...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-corrective-action-update-run-${timestamp()}.json`);

  console.log('\n=== Surveillance corrective action update QA ===');
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_CORRECTIVE_ACTION_UPDATE_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance corrective action update QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance corrective action update QA が成功しました。結果: ${outputPath}`);
}
