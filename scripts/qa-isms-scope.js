...
function main() {
  ...
  console.log('\n=== ISMS適用範囲 Playwright QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/isms-scope.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const outputPath = path.join(outputDir, `isms-scope-run-${timestamp()}.json`);
  ...
  if (result.status !== 0) {
    console.error(`\n❌ ISMS適用範囲 QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ ISMS適用範囲 QA が成功しました。結果: ${outputPath}`);
}
