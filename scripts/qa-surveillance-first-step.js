...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-first-step-run-${timestamp()}.json`);

  console.log('\n=== Surveillance first-step QA ===');
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_FIRST_STEP_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance first-step QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance first-step QA が成功しました。結果: ${outputPath}`);
}
