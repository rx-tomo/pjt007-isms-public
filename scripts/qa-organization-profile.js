...
function main() {
  ...
  console.log('\n=== 組織基本情報 Playwright QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/organization-profile.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const outputPath = path.join(outputDir, `organization-profile-run-${timestamp()}.json`);
  ...
  if (result.status !== 0) {
    console.error(`\n❌ 組織基本情報 QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ 組織基本情報 QA が成功しました。結果: ${outputPath}`);
}
