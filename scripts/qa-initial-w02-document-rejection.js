...
function main() {
  ...
  const outputPath = path.join(outputDir, `initial-w02-document-rejection-run-${timestamp()}.json`);

  console.log('\n=== W-02 文書差戻・再申請 QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/initial-w02-document-rejection.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      INITIAL_W02_DOCUMENT_REJECTION_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ W-02 文書差戻・再申請 QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ W-02 文書差戻・再申請 QA が成功しました。結果: ${outputPath}`);
}
