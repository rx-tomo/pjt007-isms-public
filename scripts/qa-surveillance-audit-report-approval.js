...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-audit-report-approval-run-${timestamp()}.json`);

  console.log('\n=== Surveillance audit report approval QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-audit-report-approval.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_AUDIT_REPORT_APPROVAL_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance audit report approval QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance audit report approval QA が成功しました。結果: ${outputPath}`);
}
