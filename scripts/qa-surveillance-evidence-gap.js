...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-evidence-gap-run-${timestamp()}.json`);

  console.log('\n=== Surveillance evidence gap QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-evidence-gap.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_EVIDENCE_GAP_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance evidence gap QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance evidence gap QA が成功しました。結果: ${outputPath}`);
}
