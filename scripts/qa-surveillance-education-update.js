...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-education-update-run-${timestamp()}.json`);

  console.log('\n=== Surveillance education update QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-education-update.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_EDUCATION_UPDATE_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance education update QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance education update QA が成功しました。結果: ${outputPath}`);
}
