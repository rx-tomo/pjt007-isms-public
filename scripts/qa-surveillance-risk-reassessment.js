...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-risk-reassessment-run-${timestamp()}.json`);

  console.log('\n=== Surveillance risk reassessment QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-risk-reassessment.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_RISK_REASSESSMENT_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance risk reassessment QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance risk reassessment QA が成功しました。結果: ${outputPath}`);
}
