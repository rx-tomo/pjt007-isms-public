...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-overdue-reminder-run-${timestamp()}.json`);

  console.log('\n=== Surveillance overdue/reminder QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-overdue-reminder.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_OVERDUE_REMINDER_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance overdue/reminder QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance overdue/reminder QA が成功しました。結果: ${outputPath}`);
}
