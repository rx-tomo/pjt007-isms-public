...
function main() {
  ...
  const outputPath = path.join(outputDir, `initial-w02-journey-run-${timestamp()}.json`);

  console.log('\n=== W-02後半 初回登録準備ジャーニー QA ===');
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      INITIAL_W02_JOURNEY_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ W-02後半 QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ W-02後半 QA が成功しました。結果: ${outputPath}`);
}
