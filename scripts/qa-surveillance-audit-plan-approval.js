...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-audit-plan-approval-run-${timestamp()}.json`);

  console.log('\n=== Surveillance audit plan approval QA ===');
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_AUDIT_PLAN_APPROVAL_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance audit plan approval QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance audit plan approval QA が成功しました。結果: ${outputPath}`);
}
