...
function main() {
  ...
  const outputPath = path.join(outputDir, `initial-user-lifecycle-run-${timestamp()}.json`);

  console.log('\n=== W-02 Step 5 ユーザー招待・ロール付与 QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/initial-user-lifecycle.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      INITIAL_USER_LIFECYCLE_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ W-02 Step 5 QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ W-02 Step 5 QA が成功しました。結果: ${outputPath}`);
}
