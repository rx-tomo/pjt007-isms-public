...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-document-revision-run-${timestamp()}.json`);

  console.log('\n=== Surveillance document revision QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-document-revision.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_DOCUMENT_REVISION_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance document revision QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance document revision QA が成功しました。結果: ${outputPath}`);
}
