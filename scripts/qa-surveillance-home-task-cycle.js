...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-home-task-cycle-run-${timestamp()}.json`);

  console.log('\n=== Surveillance home/task cycle QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-home-task-cycle.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_HOME_TASK_CYCLE_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance home/task cycle QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance home/task cycle QA が成功しました。結果: ${outputPath}`);
}
