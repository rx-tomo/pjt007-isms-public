...
function main() {
  ...
  const outputPath = path.join(outputDir, `initial-w02-assets-crud-run-${timestamp()}.json`);
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      INITIAL_W02_ASSETS_CRUD_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ W-02 情報資産CRUD QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ W-02 情報資産CRUD QA が成功しました。結果: ${outputPath}`);
}
