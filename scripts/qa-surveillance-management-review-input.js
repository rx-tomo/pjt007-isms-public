...
function main() {
  ...
  const outputPath = path.join(outputDir, `surveillance-management-review-input-run-${timestamp()}.json`);

  console.log('\n=== Surveillance management review input QA ===');
  const playwrightArgs = [
    'playwright',
    'test',
    'tests/e2e/surveillance-management-review-input.spec.ts',
    '--project=chromium',
    '--reporter=line',
  ...
  const result = spawnSync('npx', playwrightArgs, {
    ...
    env: {
      ...
      SURVEILLANCE_MANAGEMENT_REVIEW_INPUT_RESULT_PATH: outputPath,
    },
  });
  ...
  if (result.status !== 0) {
    console.error(`\n❌ Surveillance management review input QA が失敗しました。結果: ${outputPath}`);
    process.exit(result.status || 1);
  }

  console.log(`\n✅ Surveillance management review input QA が成功しました。結果: ${outputPath}`);
}
