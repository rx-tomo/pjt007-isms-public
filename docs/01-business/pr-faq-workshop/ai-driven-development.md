...
この文書は、Riscala AI for ISMSをBuild in Publicで公開するときに、AI駆動開発として何を公開し、何を公開しないかを整理する。公開用リポジトリのREADME、記事、開発ログ、発信文へ転用できる粒度で管理する。

## 基本方針

Riscala AI for ISMSは、オーナーがコードを直接書かず、AIエージェントと対話しながら設計、実装、検証、文書化を進めている業務アプリケーションである。
...
| 非公開の作業記録、運用メモ、内部手順 | 公開snapshotの説明に不要で、運用境界を崩す |
...
> Riscala AI for ISMSは、オーナーが事業判断とレビューを担い、AIエージェントが実装、検証、文書化を担う形で開発しています。公開snapshotでは、完成品としてではなく、AI駆動開発で業務アプリを作る過程と現在地を共有します。
...
- 非公開の作業記録、raw log、非公開メモが含まれていない。
