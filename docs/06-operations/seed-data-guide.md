...
|----------|--------|--------|--------------|----------|
| ISMS Pilot システム運営者 | `operator.practical@isms-practical.local` | `system_operator` | initial / surveillance の両方 | 管理者視点で2つのモデルテナント、組織、ユーザー、リスク、管理策、監査データを横断確認する |
...
| ロール | 名前 | メール | 組織所属 | 説明 |
|--------|------|--------|---------|------|
| **super_admin** | プラットフォーム管理者 | admin@isms-pilot.local | なし (null) | プラットフォーム全体を管理。テナント作成・削除、全監査ログ閲覧。組織に属さない |
| **system_operator** | 山田太郎 | yamada@isms-pilot.local | Dev Manufacturing (主) | 両テナントのシステム管理を担当。ITコンサルタント的立場で、全権限を保持 |
...
| ロール | メール | 組織 | 確認ポイント |
|--------|--------|------|-------------|
| super_admin | admin@isms-pilot.local | なし | テナント管理画面アクセス可 |
| system_operator | yamada@isms-pilot.local | Dev Manufacturing | 両組織データ閲覧可 |
